...
TLP_BATD    ?= /usr/local/share/tlp/bat.d
TLP_NMDSP   ?= /usr/local/lib/NetworkManager/dispatcher.d
TLP_CONFUSR ?= /usr/local/etc/tlp.conf
TLP_CONFDIR ?= /usr/local/etc/tlp.d
TLP_CONFDEF ?= /usr/local/share/tlp/defaults.conf
TLP_CONFREN ?= /usr/local/share/tlp/rename.conf
TLP_CONFDPR ?= /usr/local/share/tlp/deprecated.conf
TLP_CONF    ?= /usr/local/etc/default/tlp
TLP_SYSD    ?= /usr/local/lib/systemd/system
TLP_SDSL    ?= /usr/local/lib/systemd/system-sleep
...
TLP_ZSHCPL  ?= /usr/local/share/zsh/site-functions
TLP_FISHCPL ?= /usr/local/share/fish/vendor_completions.d
TLP_MAN     ?= /usr/local/share/man
TLP_META    ?= /usr/local/share/metainfo
TLP_RUN     ?= /usr/local/tmp/tlp
TLP_VAR     ?= /var/lib/tlp
